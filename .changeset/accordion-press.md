...
항목은 화살표 버튼 전체가 `--nui-scale-96` 으로 줄어듭니다. 시간과 곡선은 다른 컴포넌트의
눌림과 같은 `--nui-duration-pressed` · `--nui-easing-pressed` 입니다.
...
`__button` 을 덮어야 합니다. 그 결과로 열린 카드형 항목에서는 hover 면의 아래 두 모서리가
항목의 둥글기를 따릅니다 — 포커스 링과 같은 모양입니다.

공개 API · 클래스 이름 · CSS 변수 이름은 바뀌지 않았습니다.
