...
| 범위 | 방법 |
| --- | --- |
| 역할 하나 | `:root { --nui-action-primary: #ff6b00 }` — 그 역할을 쓰는 모든 곳이 함께 |
| 한 컴포넌트만 | `.my-tooltip { background: #222; color: #fff }` |
| 전체 | 브랜드 색 생성기 (준비 중) |

두 번째가 핵심입니다. `className` 으로 바꾸면 배경과 글자를 같은 자리에 쓰게 되므로
짝을 놓치기 어렵습니다. **`!important` 는 필요 없습니다** — 우리 CSS 는 전부
`@layer nui.*` 안에 있어 여러분의 레이어 밖 CSS 가 항상 이깁니다.
