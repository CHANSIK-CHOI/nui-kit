 ...
 *
 * 손으로 쓸 수 없는 문서다 — 185색 × 라이트·다크 × 색 33개 = 12,210 개다.
...
const labelColor = (hex, bg) => {
  ...
  return contrast(solid, "#ffffff") >= contrast(solid, "#000000") ? "#fff" : "#000";
};
...
const ALPHA = { brand: [4, 5, 6, 7], gray: [3, 4, 5, 6, 7] };
...
const rows = presets.map((p) => {
  ...
  return {
    ...
    brandAlpha: ALPHA.brand.map((s) => P(r.light.brand[`a${s}`], r.dark.brand[`a${s}`])),
    grayAlpha: ALPHA.gray.map((s) => P(r.light.gray[`a${s}`], r.dark.gray[`a${s}`])),
...
const groups = GROUPS.map((g) => ({
  ...
  sample: rows.filter((r) => r.group === g.slug).map((r) => r.solid.l)[0] ?? null,
})).filter((g) => g.count > 0);
...
writeFileSync(
  OUT,
  JSON.stringify({ count, rule, source, steps: STEPS, alpha: ALPHA, groups, presets: rows }) + "\n",
  "utf8",
);
