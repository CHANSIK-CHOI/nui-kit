...

{
  ...
  for (const [label, url, selector, open, exception] of TOUCH_TARGETS.filter(
    ...
    // 나와 44 가 43 이 된다. 등장 모션의 상한(duration-6 = 300ms)을 넘겨 기다린다 —
    // 예전에는 `duration-8 = 400ms` 를 근거로 적었는데 그 토큰은 2026-09-11 에 없어졌다
    // (tokens.md §3-5). 500 은 지금 상한의 1.6배라 그대로 둔다.
    //
    // ⚠️ 고정 대기라 메인 스레드가 바쁘면 도중을 잡을 수 있다 — `check-press` 가 그래서
    //    「값이 멈출 때까지」로 바꿨다(scripts.md §10). 여기는 대상이 25개라 재시도를
    //    걸면 검사가 크게 느려지고, 지금까지 흔들린 적이 없어 고정 대기를 유지한다.
    //    흔들리기 시작하면 그때 같은 방식으로 옮긴다.
